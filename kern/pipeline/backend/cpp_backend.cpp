...
bool buildStandaloneExe(const CppBackendResult& input, const SplConfig& config, const std::string& workspaceRoot, std::string& error) {
    ...
    cm << "  \"" << w << "/kern/core/compiler/lexer.cpp\"\n";
    cm << "  \"" << w << "/kern/core/compiler/parser.cpp\"\n";
    cm << "  \"" << w << "/kern/core/compiler/codegen.cpp\"\n";
    cm << "  \"" << w << "/kern/core/bytecode/value.cpp\"\n";
    cm << "  \"" << w << "/kern/runtime/vm/vm.cpp\"\n";
    cm << "  \"" << w << "/kern/core/errors/errors.cpp\"\n";
    ...
    cm << "  \"" << w << "/kern/modules/system/input_module.cpp\"\n";
    cm << "  \"" << w << "/kern/modules/system/vision_module.cpp\"\n";
    cm << "  \"" << w << "/kern/modules/system/render_module.cpp\"\n";
    cm << "  \"" << w << "/kern/modules/builtin_module_registry.cpp\"\n";
    if (embedWinIcon)
        cm << "  \"" << toCMakePath((tempSrc / "kern_app_icon.rc").string()) << "\"\n";
    cm << ")\n";
    cm << "target_include_directories(kernc_standalone PRIVATE \"" << w << "/src\" \"" << w << "/kern/core\" \"" << w << "/kern/pipeline\" \"" << w << "/kern/modules\" \"" << w << "/kern/runtime\" \"" << w << "\")\n";
    cm << "if(WIN32)\n";
    cm << "  target_sources(kernc_standalone PRIVATE \"" << w << "/kern/runtime/vm/http_get_winhttp.cpp\")\n";
