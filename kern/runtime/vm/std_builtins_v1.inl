...
// Uses: makeBuiltin, i, toDouble, toInt, VM, Value, ValuePtr, VMError.
    ...
    makeBuiltin(i++, [](VM*, std::vector<ValuePtr> args) {
        ...
        std::string s = std::get<std::string>(args[0]->data);
        if (s.size() % 2 != 0) return Value::fromArray(ValueArray{});
        ...
        for (size_t p = 0; p + 1 < s.size(); p += 2) {
            ...
            int hi = hexv(c0);
            int lo = hexv(c1);
            if (hi < 0 || lo < 0) return Value::fromArray(ValueArray{});
    ...
    makeBuiltin(i++, [](VM*, std::vector<ValuePtr> args) {
        ...
        if (a.size() != b.size()) return Value::fromArray(ValueArray{});
        std::vector<ValuePtr> out;
        out.reserve(a.size());
    ...
    makeBuiltin(i++, [](VM*, std::vector<ValuePtr> args) {
        ...
        if (start < 0) start = 0;
        if (start >= static_cast<int64_t>(a.size())) return Value::fromArray(ValueArray{});
        size_t st = static_cast<size_t>(start);
    ...
    makeBuiltin(i++, [](VM*, std::vector<ValuePtr> args) {
        if (args.size() < 3 || !args[0] || args[0]->type != Value::Type::ARRAY) return Value::fromArray(ValueArray{});
        ...
        if (off < 0 || static_cast<size_t>(off) + 3 >= arr.size()) return Value::fromArray(ValueArray{});
        for (int b = 0; b < 4; ++b) {
    ...
    makeBuiltin(i++, [](VM*, std::vector<ValuePtr> args) {
        if (args.size() < 2 || !args[0] || args[0]->type != Value::Type::ARRAY) return Value::fromArray(ValueArray{});
        auto arr = std::get<std::vector<ValuePtr>>(args[0]->data);
        if (arr.empty()) return Value::fromArray(ValueArray{});
    ...
    makeBuiltin(i++, [](VM*, std::vector<ValuePtr> args) {
        if (args.size() < 2 || !args[1]) return Value::fromArray(ValueArray{});
    ...
    makeBuiltin(i++, [](VM*, std::vector<ValuePtr> args) {
        if (args.empty() || !args[0] || args[0]->type != Value::Type::MAP) return Value::fromMap(ValueMap{});
