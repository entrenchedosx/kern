...
static void printUsage(const char* prog) {
    std::cout << "Kern " <<
        ...
        << "  " << prog << " install [pkg]    kern-registry install (kern.json / kern.lock). Prefer `kargo install` for GitHub packages.\n"
...
/** Shown before `kern install`: clarify registry vs kargo from cwd layout. */
static void printKernInstallKargoNotice() {
    if (const char* s = kernGetEnv("KERN_SUPPRESS_INSTALL_NOTICE")) {
        if (s[0] != '\0') return;
    }
    namespace fs = std::filesystem;
...
static int verifyLockfileAgainstManifest(bool requireKernJson, const char* label, bool printSuccessOnOk) {
    ...
    if (!fs::exists("kern.lock")) {
        std::cerr << label << ": kern.lock not found (run `kern install` for kern-registry projects, or use kargo + kargo.lock)\n";
        return 1;
    }
