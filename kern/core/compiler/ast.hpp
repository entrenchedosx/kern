...
// try / postfix ? operator: expr? — desugars to early-return on error
struct TryExpr : Expr {
    ExprPtr operand;
    explicit TryExpr(ExprPtr o) : operand(std::move(o)) {}
};
...
struct DeferStmt : Stmt {
    StmtPtr statement;  // the deferred statement(s), wraps the body
    DeferStmt(StmtPtr s) : statement(std::move(s)) {}
};
