...
StmtPtr Parser::deferStatement() {
    ...
    if (match(TokenType::LBRACE)) {
        ...
        stmt = std::make_unique<ExprStmt>(expression());
    }
...
// Returns true if the given token type CANNOT start an expression
// (used to disambiguate ternary ? from postfix try ?)
static bool isExprStart(TokenType t) {
    switch (t) {
...
ExprPtr Parser::ternary() {
    ...
    if (check(TokenType::QUESTION)) {
        ...
        advance(); // peek at what follows ?
        // If the token after ? cannot start an expression, it's postfix try (expr?)
        if (isAtEnd() || !isExprStart(peek().type)) {
            current_ = saved;         // restore before ?
