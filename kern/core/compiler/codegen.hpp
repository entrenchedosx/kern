...
public:
    ...
    void addKnownGlobalFunction(const std::string& name) { knownGlobalFunctions_.insert(name); }
...
private:
    ...
    std::vector<size_t> loopScopeDepthStack_;
    std::vector<size_t> funcRootDepthStack_;
    std::unordered_set<std::string> knownGlobalFunctions_;
    ...
    void beginScope();
    void endScope();
    void emitDeferredForScopes(size_t fromDepth, size_t toDepth);
