...
void CodeGenerator::scanLambdaCapturesStmt(const Stmt* s, size_t boundary, std::unordered_set<std::string>& seen,
    ...
    if (auto* x = dynamic_cast<const DeferStmt*>(s)) {
        scanLambdaCapturesStmt(x->statement.get(), boundary, seen, order);
        return;
    }
}
...
void CodeGenerator::endScope() {
    // Emit deferred statements for this scope in LIFO order.
    // Copy the vector by value before iterating to avoid iterator invalidation
    // if emitStmt calls beginScope() which pushes to deferStack_ (reallocation).
    auto dfrs = deferStack_.back();
    for (auto it = dfrs.rbegin(); it != dfrs.rend(); ++it) {
        emitStmt(*it);
    }
...
void CodeGenerator::emitDeferredForScopes(size_t fromDepth, size_t toDepth) {
    // Emit deferred statements for scopes fromDepth down to toDepth (inclusive)
    // in reverse scope order, and within each scope in reverse statement order (LIFO).
    // Copy each scope's vector by value to avoid iterator invalidation
    // if emitStmt calls beginScope() which reallocates deferStack_.
    // Clear each scope's entries after emitting to prevent double-emission
    // from the enclosing endScope() call.
    for (size_t d = fromDepth; d >= toDepth && d < deferStack_.size(); --d) {
        auto dfrs = deferStack_[d];
        deferStack_[d].clear();
        for (auto it = dfrs.rbegin(); it != dfrs.rend(); ++it) {
            emitStmt(*it);
        }
...
void CodeGenerator::emitExpr(const Expr* e) {
    ...
    if (auto* x = dynamic_cast<const CallExpr*>(e)) {
                        ...
                        for (size_t i = 0; i < pnames.size(); ++i)
                            if (pnames[i] == a.name) { 
                                if (ordered[i] != nullptr) {
                                    throw CodegenError("Duplicate argument for parameter '" + pnames[i] + "'", currentLine_);
                                }
                                ordered[i] = a.expr.get(); 
                                break; 
                            }
                    }
                }
        ...
        if (mem) {
            // UFCS: Check if method name is a known global function
            bool isKnownGlobal = functionParams_.count(mem->member) > 0 ||
                                 globals_.count(mem->member) > 0 ||
                                 knownGlobalFunctions_.count(mem->member) > 0;
            if (isKnownGlobal) {
                // Desugar: method(obj, args...) → LOAD_GLOBAL method, emit obj, emit args, CALL argc+1
                emit(Opcode::LOAD_GLOBAL, addConstant(mem->member));
                emitExpr(mem->object.get());  // obj as first arg
                for (const auto& a : x->args) emitExpr(a.expr.get());
                currentLine_ = x->line;
                currentColumn_ = x->column;
                emit(Opcode::CALL, static_cast<size_t>(x->args.size() + 1));
            } else {
                // Standard method call: GET_FIELD + CALL
    ...
    if (auto* x = dynamic_cast<const TryExpr*>(e)) {
        // Desugar: expr? →
        //   let __tmp = expr;
        //   if (!__tmp.is_ok) return __tmp;
        //   __tmp.value
        emitExpr(x->operand.get());              // push result map
        emit(Opcode::DUP);                       // duplicate for check + value
        emit(Opcode::GET_FIELD, addConstant("is_ok"));  // push result.is_ok
        size_t successJump = emit(Opcode::JMP_IF_TRUE, size_t(0));  // if true → success
        emit(Opcode::RETURN);                    // early return the error (DUP'd on stack)
        patchJump(successJump, code_.size());    // success path lands here
        emit(Opcode::GET_FIELD, addConstant("value"));  // extract success value
        return;
    }
...
void CodeGenerator::emitStmt(const Stmt* s) {
    ...
    if (auto* x = dynamic_cast<const ForRangeStmt*>(s)) {
        ...
        emit(Opcode::STORE, static_cast<int64_t>(slot));
        emit(Opcode::JMP, loopStart);
        // Save break/continue patches before cleanup
        auto breakJumps = std::move(breakPatches_.back());
        auto continueJumps = std::move(continuePatches_.back());
        auto loopLabel = loopLabels_.back();
    ...
    if (auto* x = dynamic_cast<const WhileStmt*>(s)) {
        loopScopeDepthStack_.push_back(deferStack_.size());
        breakPatches_.emplace_back();
        continuePatches_.emplace_back();
        loopLabels_.push_back(x->label);
        loopStartStack_.push_back(0);
    ...
    if (auto* x = dynamic_cast<const DeferStmt*>(s)) {
        // Push the deferred statement onto the current scope's defer list.
        // No bytecode is emitted now — it will be emitted when the scope exits
        // or before a return/break/continue instruction.
        deferStack_.back().push_back(x->statement.get());
        return;
    }
    if (auto* x = dynamic_cast<const TryStmt*>(s)) {
    ...
    // Desugared For-In loop: for var in expr { body }
    //   →  var __for_val = expr; var __for_idx = 0;
    //      while (__for_idx < ARRAY_LEN(__for_val)) {
    //          var = __for_val[__for_idx]; __for_idx++; body
    //      }
    // For key, value in map:
    //   →  var __for_map = expr; var __for_keys = keys(__for_map); var __for_idx = 0;
    //      while (__for_idx < ARRAY_LEN(__for_keys)) {
    ...
    if (dynamic_cast<const ReturnStmt*>(s)) {
        ...
        // Emit deferred statements from current scope down to function root
        if (!funcRootDepthStack_.empty()) {
            emitDeferredForScopes(deferStack_.size() - 1, funcRootDepthStack_.back());
        }
        if (r->values.empty()) {
            emit(Opcode::CONST_NULL);
    ...
    if (auto* x = dynamic_cast<const BreakStmt*>(s)) {
        ...
        // Emit deferred statements from current scope down to loop's scope depth
        if (k < loopScopeDepthStack_.size()) {
            emitDeferredForScopes(deferStack_.size() - 1, loopScopeDepthStack_[k]);
        }
    ...
    if (auto* x = dynamic_cast<const FunctionDeclStmt*>(s)) {
        ...
        size_t entry = code_.size();
        beginScope();
        funcRootDepthStack_.push_back(deferStack_.size() - 1);
