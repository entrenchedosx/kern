...
void Lexer::scanToken() {
    ...
    switch (c) {
        ...
        case '?':
            if (peek() == '?') {
                advance();  // second '?'; '=' (if any) left for match()
                addToken(match('=') ? TokenType::COALESCE_EQ : TokenType::COALESCE);
...
void Lexer::multilineString(char quote) {
    ...
    if (!closed) {
        throw LexerError("Unterminated multiline string", line_, column_);
    }
    // current_ is after the closing """. Grab content between the opening and closing delimiters.
    if (current_ < start_ + 3) {
        throw LexerError("Unterminated multiline string", line_, column_);
    }
    std::string value = source_.substr(start_, (current_ - 3) - start_);
