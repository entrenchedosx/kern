...
/** Turn manifest dist.tarball into an absolute URL (https or file:). Relative paths resolve against the version JSON URL. */
export function resolveDistTarballUrl(manifestUrl, tarball) {
  if (!tarball) return tarball;
  const t = String(tarball).trim();
  if (/^https?:\/\//i.test(t)) return t;
  return resolveRelativeUrl(manifestUrl, t);
}
...
export async function resolveDependencyGraph({ index, registryUrl, dependencies }) {
  ...
  async function resolveOne(name, range, stack) {
    ...
    resolved[name] = {
      ...
      dist: {
        tarball: resolveDistTarballUrl(manifestResult.manifestUrl, manifest.dist.tarball),
